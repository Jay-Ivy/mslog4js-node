...
function getLogger(logger4js, options, MASK) {
  ...
  return function (req, res, next) {
    ...
    if (req._logging) return next();
		...
		if (thislogger.isLevelEnabled(level) || options.level === 'auto') {
          ...
          var combined_tokens = assemble_tokens(req, res, options.tokens || [], options.mask ? thislogger.Mask : null);
 ...
 * @return {Array}
 */
function assemble_tokens(req, res, custom_tokens, MASK) {
  ...
  var url = getUrl(req);
  if (MASK && /\?/.test(url)) {
      var urls = url.split('?'), search = urls[1];
      var param = MASK.encrypt(JSON.parse('{"' + search.replace(/=/g, '":"').replace(/&/g, '","') + '"}'));
      url = urls[0] + '?';
      var params = [];
  ...
  default_tokens.push({ token: ':url', replacement: url });
