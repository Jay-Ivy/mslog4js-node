...
const scan = (key, value) => {
    for (var type in MASK.FIELD) {
        if (new RegExp('"' + key + '"', 'i').test('"' + MASK.FIELD[type].join('","') + '"')) {
            return MASK.ENCRYPT.hasOwnProperty(type) && isFunction(MASK.ENCRYPT[type]) && MASK.ENCRYPT[type](value.toString());
        }
    }
    return value;
};
 ...
 */
addRule(TYPE.ID_CARD, ['idCard', 'idCardNo'], function(value) {
 ...
 */
addRule(TYPE.MOBILE_PHONE, ['mobile', 'mobilePhone'], function(value) {
