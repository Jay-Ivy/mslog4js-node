...
let path = require('path'),
    ...
    defaultOpts = {
        ...
        fileBackups: 100,
        connectLevel: 'info',
        connectFormat: ':remote-addr - - ":method :url HTTP/:http-version" :status :response-timems ":referrer"'
    },
    appenderCache = [];
...
log4js.use = function (app, opts) {
    ...
    if (appenders.length) {
        logType = appenders[0].category;
        if (appenders[0].typeLog || appenders[0].typePortLog) {
            var loggers = {
                trace: function () {
                },
                debug: function () {
            ...
            appenderCache = appenderCache.concat(appenders);
            config.appenders = appenderCache;
            // 注入配置
            ...
            appenders.forEach((apperder, index) => {
                ...
                // 绑定express路由中间件日志
                if (logLevel.toLowerCase() == opts.connectLevel) app.use(this.connectLogger(logger, {
                    level: opts.connectLevel,
                    format: opts.connectFormat,
                    mask: logMask
                }));
                // 注入日志方法
                ...
                addRuleHandles.push(function () {
                    logger.Mask.addRule.apply(logger, arguments)
                });
                addRulesHandles.push(function () {
                    logger.Mask.addRules.apply(logger, arguments)
                });
            ...
            config.levels[logType] = 'ALL';
        }
    }
    appenderCache = appenderCache.concat(config.appenders);
    config.appenders = appenderCache;
    // 注入配置
    this.configure(config);
    // 创建logger对象
    ...
    logger.setMask(logMask);
    // 绑定express路由中间件日志
    app.use(this.connectLogger(logger, {level: opts.connectLevel, format: opts.connectFormat, mask: logMask}));
    return logger;
